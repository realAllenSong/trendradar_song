...
- Added audio pipeline (`trendradar/audio/pipeline.py`) with:
  ...
  - IndexTTS synthesis (HTTP, HF Space `/gradio_api/call`, with gradio_client fallback), ffmpeg concatenation, and chapters JSON output.
...
- Added dependencies: `rapidfuzz`, `google-generativeai`, `gradio_client`.
